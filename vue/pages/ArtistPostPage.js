...
const ArtistPostPage = {
	...
	async created() {
		...
		if ( this.method == "post" ) {
			this.curr_users = { avatar: "", "role": "artist" };
...
<div class="card" style="width: 100%">
	<page-wrapper :response = "server_response">
	

	<div class="card-header d-flex py-0">
		<h6 v-if= " method == 'post' " class="py-3 mb-0">Додати учасника конкурсу</h6>
		<h6 v-else class="py-3 mb-0">Редагувати учасника конкурсу</h6>
		...
		<form @submit.prevent="saveForm" id="saveFormJury">
			...
			<legend class="fs-base fw-bold border-bottom pb-2 mb-3  mt-4">Основна інформація</legend>
				<div class="row mb-3">
					...
					<div class="col-lg-2">
						<avatar-inno-uploader id = 'uploads_artist_element' v-model = 'curr_users.avatar' ></avatar-inno-uploader>
					</div>
					...
					<div class="col-lg-10">
						...
						<div class="row mb-3">
							<label class="col-lg-3 col-form-label"><span class='form-asterics'>*</span>Компанiя:</label>
							<div class="col-lg-9">
								<select class="form-control" id = 'email_element' v-model = 'curr_users.companies_id' required>
									<option v-for=" item in companies" :value=" item.id " :key=" item.id ">{{item.title}}</option>
								</select>
							</div>
						</div>

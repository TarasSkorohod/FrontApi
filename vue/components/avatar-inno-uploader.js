...
Vue.component('avatar-inno-uploader', {
	...
	computed: {
		...
		tag_value() {
			...
			// return '[{"name":"iGH37VXZUM4T.jpg","size":103896,"title":"avatar-gen11737bbe76296550f669defadda12626[1]","file":"/uploads/iGH37VXZUM4T.jpg","type":"image/jpeg"}]';
			
			if ( this.value && Array.isArray( this.value ) ) {
				console.log( 'avatar is array', JSON.stringify( this.value ) );
				...
				try {
					...
					let res =  ( data !== undefined && data[0] !== undefined && data[0]["file"] !== undefined  ) ? this.value : "";

					console.log( 'avatar is json', res );
					return res;
				} catch(e) {
					console.log( 'avatar is not json', this.value );
				};		
			};		
		...
		src() {
			...
			if ( this.value && Array.isArray( this.value ) && this.value[0].file )  {
				return this.value[0].file;
			} else {
				
				try {
					res = JSON.parse( this.value );
					
					return ( res !== undefined && res[0] !== undefined && res[0]["file"] !== undefined  ) ? res[0]["file"] : "/images/default-avatar.png";
				} catch(e) {
					...
					onSave: function(base64, item, listEl, parentEl, newInputEl, inputEl) {
						...
						if (item.upload) {
							...
							$.ajax({
								...
								console.info( save_res );
								
								Vue.$emit( 'input', save_res );								
	...
	<input v-if="editable" type="file" :data-fileuploader-files="tag_value" name="files" data-fileuploader-default="/images/default-avatar.png" ref="el">
	
	<div v-else class="fileuploader fileuploader-theme-avatar">
		<input type="hidden" name="fileuploader-list-files" value="[{&quot;file&quot;:&quot;/images/default-avatar.png&quot;,&quot;is_default&quot;:true}]"><input type="file" name="files[]" data-fileuploader-default="/images/default-avatar.png" style="position: absolute; z-index: -9999; height: 0px; width: 0px; padding: 0px; margin: 0px; line-height: 0; outline: 0px; border: 0px; opacity: 0;">
		<div class="fileuploader-wrapper">
			<div class="fileuploader-items">
				<div class="fileuploader-item file-type-image file-ext-no is-default">

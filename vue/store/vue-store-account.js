	...
	actions: {
				...
				axios.get( "auth" ).then(response => {
					if ( response.data.success && response.data ) {
						...
						if ( context.state.auth ) {
							...
							if ( data.avatar !== undefined ) data.avatar_src = FW.avatarSrc( data.avatar );
							...
							context.commit("setData", { role: "public"});
						}
		...
		postAuth( context, params ) {
			...
			return new Promise((resolve, reject) => {
				...
				if ( !utils.check_required_params( ["login"], params) ) resolve( false );  
				
				
				axios.post( "/auth", params ).then(response => {
					if ( response.data.success ) {
						
						context.dispatch("success", { message: "Ви успішно увійшли в систему!"});
						router.push("/");

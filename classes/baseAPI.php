...
class baseAPI {
	...
	public function getAllScheme( $entity ) {
		$scheme_fields = $this->scheme_fields( $entity, "patch" );
		
		$this->output["data"][$entity] = iDB::rows("SELECT * FROM `{$entity}` ORDER BY id LIMIT 1000"  );					
...
FROM modifications M LEFT JOIN modification_logs ML ON (ML.modifications_id=M.id) LEFT JOIN users U ON (U.id=M.user_id)
	...
	public function postScheme( $entity ) {
		...
		if ( $params = $this->fetch_params( $scheme_fields , false ) ) {	
			...
			if ( 
				isset( $params["avatar"]) && 
				isset( $params["avatar"]["avatar_title"]) &&
				isset( $params["avatar"]["avatar"]) &&
				isset( $params["avatar"]["avatar_size"])
			) {
				$params["avatar_title"] = $params["avatar"]["avatar_title"];
				$params["avatar_size"] = $params["avatar"]["avatar_size"];
				$params["avatar"] = "/uploads/" . $params["avatar"]["avatar"];
			};
			*/
